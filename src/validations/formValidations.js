export const validators = {
  ...
  phone: (val) => {
    ...
    return /^[\d\s+\-().]{8,}$/.test(val) ? null : 'Enter a valid phone number.';
  },
  password: (val) => {
  ...
  dob: (val) => {
    ...
    return /^\d{2}\/\d{2}\/\d{4}$/.test(val) ? null : 'Use DD/MM/YYYY format.';
  },
  positiveNumber: (val) => {
    const n = Number(val);
    return (!val || isNaN(n) || n < 0) ? 'Enter a valid positive number.' : null;
  },
};
